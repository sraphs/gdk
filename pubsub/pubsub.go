...
//
// See https://github.com/sraphs/gdk/howto/pubsub/ for a detailed how-to guide.
//
...
// For example, "github.com/sraphs/gdk/pubsub/Topic.Send".
...
// For example, "github.com/sraphs/gdk/pubsub/latency".
//
...
import (
	...
	"github.com/sraphs/gdk/internal/retry"
...
// drivers; see
// https://godoc.org/github.com/sraphs/gdk/pubsub#hdr-At_most_once_and_At_least_once_Delivery
// for more info.
//
...
func (t *Topic) Send(ctx context.Context, m *Message) (err error) {
	...
	if m.LoggableID != "" {
		return gdkerr.Newf(gdkerr.InvalidArgument, nil, "pubsub: Message.LoggableID should not be set when sending a message")
	}
	for k, v := range m.Metadata {
		if !utf8.ValidString(k) {
			return gdkerr.Newf(gdkerr.InvalidArgument, nil, "pubsub: Message.Metadata keys must be valid UTF-8 strings: %q", k)
		}
		if !utf8.ValidString(v) {
			return gdkerr.Newf(gdkerr.InvalidArgument, nil, "pubsub: Message.Metadata values must be valid UTF-8 strings: %q", v)
		}
	}
	dm := &driver.Message{
		Body:       m.Body,
...
var errTopicShutdown = gdkerr.Newf(gdkerr.FailedPrecondition, nil, "pubsub: Topic has been Shutdown")
...
const pkgName = "github.com/sraphs/gdk/pubsub"
...
var errSubscriptionShutdown = gdkerr.Newf(gdkerr.FailedPrecondition, nil, "pubsub: Subscription has been Shutdown")
...
func (s *Subscription) ErrorAs(err error, i interface{}) bool {
	return gdkerr.ErrorAs(err, i, s.driver.ErrorAs)
}
...
func wrapError(ec errorCoder, err error) error {
	...
	return gdkerr.New(ec.ErrorCode(err), err, 2, "pubsub")
}

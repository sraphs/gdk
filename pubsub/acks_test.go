...
func (*ackingDriverSub) ErrorCode(error) gdkerr.ErrorCode { return gdkerr.Internal }
...
func TestReceiveReturnsErrorFromSendAcks(t *testing.T) {
	...
	for {
		...
		if gdkerr.Code(err) == gdkerr.Internal && err.(*gdkerr.Error).Unwrap() == serr {
			break // success
		}
		if err != nil {
...
func (*callbackDriverSub) ErrorCode(error) gdkerr.ErrorCode { return gdkerr.Internal }
...
func TestReceiveReturnsAckErrorOnNoMoreMessages(t *testing.T) {
	...
	if got := gdkerr.Code(err); got != gdkerr.Internal {
		t.Fatalf("error code = %v; want %v", got, gdkerr.Internal)
	}

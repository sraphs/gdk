...
func testNonExistentTopicSucceedsOnOpenButFailsOnSend(t *testing.T, newHarness HarnessMaker) {
	...
	if err == nil || gdkerr.Code(err) != gdkerr.NotFound {
...
func testNonExistentSubscriptionSucceedsOnOpenButFailsOnReceive(t *testing.T, newHarness HarnessMaker) {
	...
	if err == nil || ctx2.Err() != nil || gdkerr.Code(err) != gdkerr.NotFound {
...
func isCanceled(err error) bool {
	...
	return gdkerr.Code(err) == gdkerr.Canceled
}
...
func testAs(t *testing.T, newHarness HarnessMaker, st AsTest) {
	...
	if topicErr == nil || gdkerr.Code(topicErr) != gdkerr.NotFound {
	...
	if subErr == nil || ctx2.Err() != nil || gdkerr.Code(subErr) != gdkerr.NotFound {

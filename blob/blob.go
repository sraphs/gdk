...
//
// See https://github.com/sraphs/gdk/howto/blob/ for a detailed how-to guide.
//
// # Errors
//
// The errors returned from this package can be inspected in several ways:
//
// The Code function from github.com/sraphs/gdk/gdkerr will return an error code, also
...
// For example, "github.com/sraphs/gdk/blob/Attributes".
...
// For example, "github.com/sraphs/gdk/blob/latency".
//
// It also collects the following metrics:
//   - github.com/sraphs/gdk/blob/bytes_read: the total number of bytes read, by driver.
//   - github.com/sraphs/gdk/blob/bytes_written: the total number of bytes written, by driver.
//
		...
		if r.relativeOffset == saved {
			...
			if length >= 0 {
				...
				if length < 0 {
					...
					return 0, gdkerr.Newf(gdkerr.Internal, nil, "blob: invalid Seek (base length %d, relative offset %d)", r.baseLength, r.relativeOffset)
				}
			}
			var err error
...
func (w *Writer) Close() (err error) {
	...
	if len(w.contentMD5) > 0 {
		...
		if !bytes.Equal(md5sum, w.contentMD5) {
			...
			return gdkerr.Newf(gdkerr.FailedPrecondition, nil, "blob: the WriterOptions.ContentMD5 you specified (%X) did not match what was written (%X)", w.contentMD5, md5sum)
		}
	}
...
const pkgName = "github.com/sraphs/gdk/blob"
...
func (b *Bucket) ListPage(ctx context.Context, pageToken []byte, pageSize int, opts *ListOptions) (retval []*ListObject, nextPageToken []byte, err error) {
	...
	if pageSize <= 0 {
		return nil, nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: pageSize must be > 0")
	}
...
// gdkerr.Code will return gdkerr.NotFound.
func (b *Bucket) Attributes(ctx context.Context, key string) (_ *Attributes, err error) {
	if !utf8.ValidString(key) {
		return nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: Attributes key must be a valid UTF-8 string: %q", key)
	}
...
// gdkerr.Code will return gdkerr.NotFound. Exists is a lighter-weight way
...
func (b *Bucket) newRangeReader(ctx context.Context, key string, offset, length int64, opts *ReaderOptions) (_ *Reader, err error) {
	...
	if offset < 0 {
		return nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: NewRangeReader offset must be non-negative (%d)", offset)
	}
	if !utf8.ValidString(key) {
		return nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: NewRangeReader key must be a valid UTF-8 string: %q", key)
	}
	if opts == nil {
		opts = &ReaderOptions{}
	}
...
func (b *Bucket) NewWriter(ctx context.Context, key string, opts *WriterOptions) (_ *Writer, err error) {
	if !utf8.ValidString(key) {
		return nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: NewWriter key must be a valid UTF-8 string: %q", key)
	}
	if opts == nil {
		opts = &WriterOptions{}
	}
	...
	if len(opts.Metadata) > 0 {
		...
		for k, v := range opts.Metadata {
			if k == "" {
				return nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: WriterOptions.Metadata keys may not be empty strings")
			}
			if !utf8.ValidString(k) {
				return nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: WriterOptions.Metadata keys must be valid UTF-8 strings: %q", k)
			}
			if !utf8.ValidString(v) {
				return nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: WriterOptions.Metadata values must be valid UTF-8 strings: %q", v)
			}
			lowerK := strings.ToLower(k)
			if _, found := md[lowerK]; found {
				return nil, gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: WriterOptions.Metadata has a duplicate case-insensitive metadata key: %q", lowerK)
			}
			md[lowerK] = v
		}
		dopts.Metadata = md
...
func (b *Bucket) Copy(ctx context.Context, dstKey, srcKey string, opts *CopyOptions) (err error) {
	if !utf8.ValidString(srcKey) {
		return gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: Copy srcKey must be a valid UTF-8 string: %q", srcKey)
	}
	if !utf8.ValidString(dstKey) {
		return gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: Copy dstKey must be a valid UTF-8 string: %q", dstKey)
	}
	if opts == nil {
		opts = &CopyOptions{}
	}
...
func (b *Bucket) Delete(ctx context.Context, key string) (err error) {
	if !utf8.ValidString(key) {
		return gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: Delete key must be a valid UTF-8 string: %q", key)
	}
	b.mu.RLock()
	defer b.mu.RUnlock()
	if b.closed {
...
// will return an error for which gdkerr.Code will return gdkerr.Unimplemented.
func (b *Bucket) SignedURL(ctx context.Context, key string, opts *SignedURLOptions) (string, error) {
	if !utf8.ValidString(key) {
		return "", gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: SignedURL key must be a valid UTF-8 string: %q", key)
	}
	...
	switch {
	case opts.Expiry < 0:
		return "", gdkerr.Newf(gdkerr.InvalidArgument, nil, "blob: SignedURLOptions.Expiry must be >= 0 (%v)", opts.Expiry)
	case opts.Expiry == 0:
...
func wrapError(b driver.Bucket, err error, key string) error {
	...
	if code == gdkerr.Unknown {
		code = b.ErrorCode(err)
	}
	return gdkerr.New(code, err, 2, msg)
}

var errClosed = gdkerr.Newf(gdkerr.FailedPrecondition, nil, "blob: Bucket has been closed")

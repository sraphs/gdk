...
func (b *fakeAttributes) ErrorCode(err error) gdkerr.ErrorCode {
	if err == errNotFound {
...
func (*fakeLister) ErrorCode(err error) gdkerr.ErrorCode { return gdkerr.Unknown }
...
func (b *erroringBucket) ErrorCode(err error) gdkerr.ErrorCode {
	return gdkerr.Unknown
}
...
func TestURLMux(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "query options",
			url:  "foo://my-bucket?aAa=bBb&cCc=dDd",
		},
		{
			name: "multiple query options",
			url:  "foo://my-bucket?x=a&x=b&x=c",
		},
		{

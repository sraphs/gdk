...
func testAttributes(t *testing.T, newHarness HarnessMaker) {
	...
	for _, badKey := range []string{
		...
		} else if gdkerr.Code(err) != gdkerr.NotFound {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				...
				if tc.exists {
					...
					} else if !tc.wantReadErr && gdkerr.Code(err) != gdkerr.NotFound {
...
func testKeys(t *testing.T, newHarness HarnessMaker) {
	...
	for description, key := range escape.WeirdStrings {
		t.Run(description, func(t *testing.T) {
			...
			if gdkerr.Code(err) != gdkerr.Unimplemented {
				if err != nil {
					t.Error(err)
				}
				client := h.HTTPClient()
...
func testAs(t *testing.T, newHarness HarnessMaker, st AsTest) {
	...
	for _, method := range []string{http.MethodGet, http.MethodPut, http.MethodDelete} {
		...
		if err != nil && gdkerr.Code(err) != gdkerr.Unimplemented {

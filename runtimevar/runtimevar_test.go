...
func (*fakeWatcher) ErrorCode(error) gdkerr.ErrorCode { return gdkerr.Internal }
...
func (b *erroringWatcher) ErrorCode(err error) gdkerr.ErrorCode {
	return gdkerr.Internal
}
...
func TestErrorsAreWrapped(t *testing.T) {
	...
	verifyWrap := func(description string, err error) {
		if unwrapped, ok := err.(*gdkerr.Error); !ok {
			t.Errorf("%s: not wrapped: %v", description, err)
		} else if du, ok := unwrapped.Unwrap().(*gdkerr.Error); ok {
